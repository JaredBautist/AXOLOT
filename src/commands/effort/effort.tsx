...
import { type EffortValue, getDisplayedEffortLevel, getEffortEnvOverride, getEffortValueDescription, isEffortLevel, normalizeEffortLevelInput, toPersistableEffort } from '../../utils/effort.js';
...
export function executeEffort(args: string): EffortCommandResult {
  const normalized = normalizeEffortLevelInput(args);
  if (normalized === 'auto') {
    return unsetEffortLevel();
  }
  if (!isEffortLevel(normalized)) {
    return {
      message: `Invalid argument: ${args}. Valid options are: normal, low, medium, high, max, auto`
    };
  }
...
export async function call(onDone: LocalJSXCommandOnDone, _context: unknown, args?: string): Promise<React.ReactNode> {
  ...
  if (COMMON_HELP_ARGS.includes(args)) {
    onDone('Usage: /effort [normal|low|medium|high|max|auto]\n\nEffort levels:\n- normal: Balanced default, same as medium\n- low: Quick responses with the lowest overhead\n- medium: Balanced approach with standard testing\n- high: Comprehensive implementation with extensive testing\n- max: Maximum capability with deepest reasoning (Opus 4.6 only)\n- auto: Use the upstream default effort level for your model');
    return;
  }
