...
Manages session state in \`.claudex/session.json\` so you can pick up where you left off across sessions.
...
Save current session state to \`.claudex/session.json\`. Include what you were working on, files changed, and next steps.

### \`/session status\`
Show saved session state if one exists.

### \`/session resume\`
Read \`.claudex/session.json\` and restore context: load files mentioned in "files", re-read relevant code, and continue the task.
...
export function registerSessionSkill(): void {
  registerBundledSkill({
    name: 'session',
    description:
      'Persist session state to .claudex/session.json and restore it across sessions.',
    whenToUse:
